...
import {
  en_US_default
} from "./chunk-VHW4L7U2.js";
import {
  mainnet
} from "./chunk-HBROGEA3.js";
import {
  require_react_dom
...
import {
  ...
  wNAF,
  weierstrassPoints
} from "./chunk-QAIYB4S6.js";
import "./chunk-JHJ4NL2O.js";
import {
...
// node_modules/ox/_esm/core/Base32.js
...
var bls12_381 = bls({
  ...
  G2: {
    ...
    fromBytes: (bytes) => {
      ...
      const L = Fp.BYTES;
      const slc = (b, from9, to) => bytesToNumberBE(b.slice(from9, to));
...
function twistedEdwards(curveDef) {
  ...
  const { Fp: Fp4, n: CURVE_ORDER, prehash, hash: cHash, randomBytes: randomBytes4, nByteLength, h: cofactor } = CURVE;
  ...
  const utils2 = {
    ...
    randomPrivateKey: () => randomBytes4(Fp4.BYTES),
    /**
...
function calcElligatorRistrettoMap(r0) {
  ...
  if (!Ns_D_is_sq)
    c = r2;
  const Nt = mod4(c * (r2 - _1n6) * D_MINUS_ONE_SQ - D);
  const s2 = s * s;
  const W0 = mod4((s + s) * D);
  const W1 = mod4(Nt * SQRT_AD_MINUS_ONE);
  const W2 = mod4(_1n6 - s2);
  const W3 = mod4(_1n6 + s2);
  return new ed25519.ExtendedPoint(mod4(W0 * W3), mod4(W2 * W1), mod4(W1 * W3), mod4(W0 * W2));
}
...
function FpPow2(Fp4, num2, power) {
  if (power < _0n8)
...
function FpLegendre2(Fp4, n) {
  const p1mod2 = (Fp4.ORDER - _1n8) / _2n7;
...
function Field2(ORDER, bitLenOrOpts, isLE4 = false, opts = {}) {
  ...
  const { nBitLength: BITS, nByteLength: BYTES2 } = nLength(ORDER, _nbitLength);
  if (BYTES2 > 2048)
  ...
  const f = Object.freeze({
    ...
    toBytes: (num2) => isLE4 ? numberToBytesLE2(num2, BYTES2) : numberToBytesBE2(num2, BYTES2),
    fromBytes: (bytes, skipValidation = true) => {
      if (allowedLengths) {
        if (!allowedLengths.includes(bytes.length) || bytes.length > BYTES2) {
          throw new Error("Field.fromBytes: expected " + allowedLengths + " bytes, got " + bytes.length);
        }
        const padded = new Uint8Array(BYTES2);
...
var divNearest = (num2, den) => (num2 + (num2 >= 0 ? den : -den) / _2n8) / den;
...
function weierstrassN(params, extraOpts = {}) {
  ...
  if (!isValidXY(CURVE.Gx, CURVE.Gy))
    throw new Error("bad curve params: generator point");
  const _4a3 = Fp4.mul(Fp4.pow(CURVE.a, _3n6), _4n3);
...
function SWUFpSqrtRatio(Fp4, Z) {
  ...
  const c7 = Fp4.pow(Z, (c2 + _1n10) / _2n8);
  let sqrtRatio = (u, v) => {
    let tv1 = c6;
  ...
  if (Fp4.ORDER % _4n3 === _3n6) {
    const c12 = (Fp4.ORDER - _3n6) / _4n3;
...
function ecdsa(Point2, hash, ecdsaOpts = {}) {
  ...
  const hmac4 = ecdsaOpts.hmac || ((key, ...msgs) => hmac(hash, key, concatBytes3(...msgs)));
  ...
  class Signature {
    ...
    toBytes(format2 = defaultSigOpts_format) {
      ...
      if (format2 === "recovered") {
        ...
        return concatBytes3(Uint8Array.of(this.recovery), r2, s);
      }
      return concatBytes3(r2, s);
    }
    toHex(format2) {
  ...
  function prepSig(message, privateKey, opts) {
    ...
    function k2sig(kBytes) {
      ...
      if (s === _0n10)
        return;
      let recovery = (q.x === r2 ? 0 : 2) | Number(q.y & _1n10);
      let normS = s;
...
function createCurve(curveDef, defHash) {
  const create4 = (hash) => weierstrass({ ...curveDef, hash });
...
var _DST_scalar = utf8ToBytes2("HashToScalar-");
...
var secp256k1 = createCurve({ ...secp256k1_CURVE, Fp: Fpk1, lowS: true, endo: secp256k1_ENDO }, sha2563);
var TAGGED_HASH_PREFIXES = {};
function taggedHash(tag, ...messages) {
  let tagP = TAGGED_HASH_PREFIXES[tag];
  if (tagP === void 0) {
    const tagH = sha2563(utf8ToBytes2(tag));
...
function lift_x(x) {
  const Fp4 = Fpk1;
  if (!Fp4.isValidNot0(x))
    throw new Error("invalid x: Fail if x ≥ p");
  const xx = Fp4.create(x * x);
  const c = Fp4.create(xx * x + BigInt(7));
  let y = Fp4.sqrt(c);
  if (!hasEven(y))
    y = Fp4.neg(y);
...
function schnorrVerify(signature, message, publicKey) {
  ...
  try {
    ...
    if (!inRange(r2, _1n11, secp256k1_CURVE.p))
      return false;
    const s = num(sig.subarray(32, 64));
    if (!inRange(s, _1n11, secp256k1_CURVE.n))
      return false;
    const e = challenge(Fn.toBytes(r2), pointToBytes(P), m);
...
var secp256k1_hasher = (() => createHasher3(secp256k1.Point, (scalars) => {
...
function join(separator = "") {
  ...
  return {
    encode: (from9) => {
      astrArr("join.decode", from9);
      return from9.join(separator);
    },
    decode: (to) => {
      astr("join.decode", to);
...
function normalize(fn) {
  afn(fn);
  return { encode: (from9) => from9, decode: (to) => fn(to) };
}
function convertRadix(data, from9, to) {
  if (from9 < 2)
    throw new Error(`convertRadix: invalid from=${from9}, base cannot be less than 2`);
  if (to < 2)
  ...
  while (true) {
    ...
    for (let i = pos; i < dlen; i++) {
      ...
      const fromCarry = from9 * carry;
      const digitBase = fromCarry + digit;
      if (!Number.isSafeInteger(digitBase) || fromCarry / from9 !== carry || digitBase - digit !== fromCarry) {
...
var radix2carry = (from9, to) => from9 + (to - gcd(from9, to));
var powers = (() => {
  let res = [];
  for (let i = 0; i < 40; i++)
    res.push(2 ** i);
...
function convertRadix2(data, from9, to, padding2) {
  aArr(data);
  if (from9 <= 0 || from9 > 32)
    throw new Error(`convertRadix2: wrong from=${from9}`);
  if (to <= 0 || to > 32)
    throw new Error(`convertRadix2: wrong to=${to}`);
  if (radix2carry(from9, to) > 32) {
    throw new Error(`convertRadix2: carry overflow from=${from9} to=${to} carryBits=${radix2carry(from9, to)}`);
  }
  let carry = 0;
  let pos = 0;
  const max = powers[from9];
  const mask = powers[to] - 1;
  const res = [];
  for (const n of data) {
    anumber3(n);
    if (n >= max)
      throw new Error(`convertRadix2: invalid data word=${n} from=${from9}`);
    carry = carry << from9 | n;
    if (pos + from9 > 32)
      throw new Error(`convertRadix2: carry overflow pos=${pos} from=${from9}`);
    pos += from9;
    for (; pos >= to; pos -= to)
...
function ctr32(xk, isLE4, nonce, src7, dst) {
  abytes4(nonce, BLOCK_SIZE2);
  abytes4(src7);
...
function computeTag(fn, isLE4, key, data, AAD) {
  ...
  if (AAD)
    h.update(AAD);
  const num2 = u64Lengths(8 * data.length, 8 * aadLength, isLE4);
  h.update(data);
  h.update(num2);
  const res = h.digest();
  clean3(num2);
...
var aeskw = wrapCipher({ blockSize: 8 }, (kek) => ({
  encrypt(plaintext) {
    ...
    const out = concatBytes4(AESKW_IV, plaintext);
...
var hmac3 = (hash, key, message) => new HMAC2(hash, key).update(message).digest();
hmac3.create = (hash, key) => new HMAC2(hash, key);
...
var sha5123 = createHasher4(() => new SHA5122());
...
function __spreadArray(to, from9, pack) {
  if (pack || arguments.length === 2) for (var i = 0, l = from9.length, ar; i < l; i++) {
    if (ar || !(i in from9)) {
      if (!ar) ar = Array.prototype.slice.call(from9, 0, i);
      ar[i] = from9[i];
    }
  }
...
function alphabet3(alphabet4) {
  return {
    has: (char) => alphabet4.includes(char),
    decode: (input) => {
...
var Bitmap = class _Bitmap {
  ...
  xy(c) {
    ...
    c.x = mod3(c.x, this.width);
    c.y = mod3(c.y, this.height);
    return c;
  }
  /**
  ...
  set(x, y, v) {
    if (v === void 0)
      return;
    this.maskWord(this.wordIndex(x, y), bitMask3(x), v);
  }
  ...
  border(border2 = 2, value) {
    ...
    out.embed({ x: border2, y: border2 }, this);
    return out;
  }
  // Embed another bitmap on coordinates
  embed(c, src7) {
    const { x, y } = this.xy(c);
    const { height, width } = _Bitmap.size(src7.size(), this.size({ x, y }));
    if (width <= 0 || height <= 0)
      return this;
    const { value, defined } = this;
    const { words: srcStride, value: srcValue } = src7;
    for (let yPos = 0; yPos < height; yPos++) {
      const srcRow = yPos * srcStride;
      for (let xPos = 0; xPos < width; ) {
        const dstX = x + xPos;
        const { word: dstWord, bit: dstBit } = this.bitIndex(dstX, y + yPos);
        const { word: srcWord, bit: srcBit } = src7.bitIndex(xPos, yPos);
    ...
    for (let by = 0; by < height; by += 32) {
      for (let bx = 0; bx < words; bx++) {
        ...
        for (let r2 = 0; r2 < rows; r2++) {
          const wi = this.wordIndex(32 * bx, by + r2);
          tmpV[r2] = value[wi];
          tmpD[r2] = defined[wi];
        }
        tmpV.fill(0, rows);
...
var info = {
  ...
  alphabet: {
    numeric: alphabet3("0123456789"),
    alphanumerc: alphabet3("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:")
  },
...
function drawTemplate(ver, ecc, maskIdx, test = false) {
  ...
  b = b.hLine({ x: 0, y: 6 }, Infinity, ({ x }) => b.isDefined(x, 6) ? void 0 : x % 2 == 0).vLine({ x: 6, y: 0 }, Infinity, ({ y }) => b.isDefined(6, y) ? void 0 : y % 2 == 0);
  {
    ...
    for (let i = 8; i < 15; i++)
      b.set(8, size3 - 15 + i, getBit(i));
    for (let i = 0; i < 8; i++)
      b.set(size3 - i - 1, 8, getBit(i));
    for (let i = 8; i < 9; i++)
      b.set(15 - i - 1 + 1, 8, getBit(i));
    for (let i = 9; i < 15; i++)
      b.set(15 - i - 1, 8, getBit(i));
    b.set(8, size3 - 8, !test);
  }
  if (ver >= 7) {
...
function encode2(ver, ecc, data, type, encoder = utf8ToBytes5) {
  let encoded = "";
  ...
  const padding2 = "1110110000010001";
  for (let idx = 0; bits.length !== capacity; idx++)
    bits += padding2[idx % padding2.length];
...
function encodeQR(text, output = "raw", opts = {}) {
  ...
  if (ver !== void 0) {
    validateVersion(ver);
    data = encode2(ver, ecc, text, encoding, opts.textEncoder);
  } else {
    for (let i = 1; i <= 40; i++) {
      try {
        data = encode2(i, ecc, text, encoding, opts.textEncoder);
        ver = i;
        break;
      } catch (e) {
        err = e;
...
(function(Cuer2) {
  ...
  function Root(props) {
    const { children, size: size3 = "100%", value, version: version2, errorCorrection, ...rest } = props;
    ...
    async getProvider() {
      if (!walletProvider) {
        ...
        const { createBaseAccountSDK } = await (() => {
          try {
            return import("./connectors_false-56EKMIIW.js");
          } catch {
        ...
        if (error.code === 4902) {
          try {
            ...
            if (addEthereumChainParameter?.blockExplorerUrls)
              ...
              blockExplorerUrls = chain2.blockExplorers?.default.url ? [chain2.blockExplorers?.default.url] : [];
            let rpcUrls;
            if (addEthereumChainParameter?.rpcUrls?.length)
              rpcUrls = addEthereumChainParameter.rpcUrls;
            else
              rpcUrls = [chain2.rpcUrls.default?.http[0] ?? ""];
            const addEthereumChain = {
              blockExplorerUrls,
              chainId: numberToHex(chainId),
              chainName: addEthereumChainParameter?.chainName ?? chain2.name,
              iconUrls: addEthereumChainParameter?.iconUrls,
              nativeCurrency: addEthereumChainParameter?.nativeCurrency ?? chain2.nativeCurrency,
              rpcUrls
            };
            await provider.request({
    ...
    async getProvider() {
      if (!walletProvider) {
        const { createCoinbaseWalletSDK } = await (() => {
          try {
            return import("./connectors_false-7WSD3ERE.js");
          } catch {
      ...
      async function initProvider() {
        const MetaMaskSDK = await (async () => {
          const { default: SDK } = await (() => {
            try {
              return import("./connectors_false-4CT2DGJQ.js");
            } catch {
              ...
              params: [
                {
                  ...
                  rpcUrls: (() => {
                    ...
                    return [chain2.rpcUrls.default?.http[0] ?? ""];
                  })()
                }
              ]
            });
  ...
  return createConnector((config) => ({
    ...
    async getProvider() {
      ...
      if (!provider_) {
        const { default: SDK } = await (() => {
          try {
            return import("./connectors_false-ZCWHKDLC.js");
          } catch {
        ...
        const SafeAppProvider = await (async () => {
          const Provider = await (() => {
            try {
              return import("./connectors_false-ZBD2PFSX.js");
            } catch {
    ...
    async getProvider({ chainId } = {}) {
      async function initProvider() {
        ...
        const { EthereumProvider } = await (() => {
          try {
            return import("./connectors_false-SWTADV42.js");
          } catch {
...
var fetchTranslations = async (locale) => {
  switch (locale) {
    case "ar":
    case "ar-AR":
      return (await import("./ar_AR-44JUQ6EW-BAZ7VNPS.js")).default;
    case "de":
    case "de-DE":
      return (await import("./de_DE-PYAO5YD6-P5XTQT3L.js")).default;
    case "en":
    case "en-US":
      return (await import("./en_US-Y4ZOVFV4-TV3MC42B.js")).default;
    case "es":
    case "es-419":
      return (await import("./es_419-HRGOJ6VT-EDKWBFPR.js")).default;
    case "fr":
    case "fr-FR":
      return (await import("./fr_FR-IGGVC6RM-BKTQ7OGD.js")).default;
    case "hi":
    case "hi-IN":
      return (await import("./hi_IN-XSKAA3MF-RLG2YUK4.js")).default;
    case "id":
    case "id-ID":
      return (await import("./id_ID-KUOB4UAL-TQNROVKA.js")).default;
    case "ja":
    case "ja-JP":
      return (await import("./ja_JP-4WRQ63RX-4OKLWKLN.js")).default;
    case "ko":
    case "ko-KR":
      return (await import("./ko_KR-I2K7AIEP-ZIXG3EMZ.js")).default;
    case "ms":
    case "ms-MY":
      return (await import("./ms_MY-6VZW6Q3Z-T2JBI6V3.js")).default;
    case "pt":
    case "pt-BR":
      return (await import("./pt_BR-N3XOZ4SV-V7T3CKXP.js")).default;
    case "ru":
    case "ru-RU":
      return (await import("./ru_RU-IITAPRET-NQDM4SAN.js")).default;
    case "th":
    case "th-TH":
      return (await import("./th_TH-3ZQCKCQE-NGN77DY7.js")).default;
    case "tr":
    case "tr-TR":
      return (await import("./tr_TR-QUPUGMUF-WVSUFIQY.js")).default;
    case "ua":
    case "uk-UA":
      return (await import("./uk_UA-SSN6SV5E-QJYD2Y5T.js")).default;
    case "vi":
    case "vi-VN":
      return (await import("./vi_VN-7BUVRNYY-UIQ2346M.js")).default;
    case "zh":
    case "zh-CN":
    case "zh-Hans":
      return (await import("./zh_CN-HJBN674C-WXQ3TFQ7.js")).default;
    case "zh-HK":
      return (await import("./zh_HK-JW3WLRKE-PXN6GFW4.js")).default;
    case "zh-Hant":
    case "zh-TW":
      return (await import("./zh_TW-VNNRNXY7-EEDUS4H4.js")).default;
    default:
      return (await import("./en_US-Y4ZOVFV4-TV3MC42B.js")).default;
  }
};
...
var apeChainIcon = {
  ...
  iconUrl: async () => (await import("./apechain-SX5YFU6N-M37SY3YH.js")).default
};
var arbitrumIcon = {
  iconBackground: "#96bedc",
  iconUrl: async () => (await import("./arbitrum-WURIBY6W-I5B3W575.js")).default
};
var avalancheIcon = {
  iconBackground: "#e84141",
  iconUrl: async () => (await import("./avalanche-KOMJD3XY-RZVXZHE4.js")).default
};
var baseIcon = {
  iconBackground: "#0052ff",
  iconUrl: async () => (await import("./base-OAXLRA4F-57JWZJ6Q.js")).default
};
var berachainIcon = {
  iconBackground: "#814625",
  iconUrl: async () => (await import("./berachain-NJECWIVC-UEXBPDVG.js")).default
};
var blastIcon = {
  iconBackground: "#000000",
  iconUrl: async () => (await import("./blast-V555OVXZ-MOORSH7T.js")).default
};
var bscIcon = {
  iconBackground: "#ebac0e",
  iconUrl: async () => (await import("./bsc-N647EYR2-XILTKQHQ.js")).default
};
var celoIcon = {
  iconBackground: "#FCFF52",
  iconUrl: async () => (await import("./celo-GEP4TUHG-HECQD5YZ.js")).default
};
var cronosIcon = {
  iconBackground: "#002D74",
  iconUrl: async () => (await import("./cronos-HJPAQTAE-RHDK64D5.js")).default
};
var degenIcon = {
  iconBackground: "#A36EFD",
  iconUrl: async () => (await import("./degen-FQQ4XGHB-JGZ7X2DH.js")).default
};
var ethereumIcon = {
  iconBackground: "#484c50",
  iconUrl: async () => (await import("./ethereum-RGGVA4PY-46WC4QAO.js")).default
};
var flowIcon = {
  iconBackground: "transparent",
  iconUrl: async () => (await import("./flow-5FQJFCTK-PULWZSFH.js")).default
};
var gnosisIcon = {
  iconBackground: "#04795c",
  iconUrl: async () => (await import("./gnosis-37ZC4RBL-FEJR3RZT.js")).default
};
var gravityIcon = {
  iconBackground: "#000000",
  iconUrl: async () => (await import("./gravity-J5YQHTYH-L3NMT5RO.js")).default
};
var hardhatIcon = {
  iconBackground: "#f9f7ec",
  iconUrl: async () => (await import("./hardhat-TX56IT5N-5LKHTQ5O.js")).default
};
var hyperevmIcon = {
  iconBackground: "#000000",
  iconUrl: async () => (await import("./hyperevm-VKPAA4SA-JUSXT53L.js")).default
};
var inkIcon = {
  iconBackground: "#7132F5",
  iconUrl: async () => (await import("./ink-FZMYZWHG-UH4ON7DT.js")).default
};
var kaiaIcon = {
  iconBackground: "transparent",
  iconUrl: async () => (await import("./kaia-65D2U3PU-65EBO4PG.js")).default
};
var lineaIcon = {
  iconBackground: "#ffffff",
  iconUrl: async () => (await import("./linea-QRMVQ5DY-46H27NVT.js")).default
};
var mantaIcon = {
  iconBackground: "#ffffff",
  iconUrl: async () => (await import("./manta-SI27YFEJ-UEFBDBYA.js")).default
};
var mantleIcon = {
  iconBackground: "#000000",
  iconUrl: async () => (await import("./mantle-CKIUT334-RI3NFHUP.js")).default
};
var monadTestnetIcon = {
  iconBackground: "transparent",
  iconUrl: async () => (await import("./monad-4KWC6TSS-WMQH6J6S.js")).default
};
var optimismIcon = {
  iconBackground: "#ff5a57",
  iconUrl: async () => (await import("./optimism-HAF2GUT7-6RBNBDWU.js")).default
};
var polygonIcon = {
  iconBackground: "#9f71ec",
  iconUrl: async () => (await import("./polygon-WW6ZI7PM-A77UF4YS.js")).default
};
var roninIcon = {
  iconBackground: "#1273EA",
  iconUrl: async () => (await import("./ronin-EMCPYXZT-A7F5D3SO.js")).default
};
var sankoIcon = {
  iconBackground: "#000000",
  iconUrl: async () => (await import("./sanko-RHQYXGM5-M74F6YIY.js")).default
};
var superpositionIcon = {
  iconBackground: "#000000",
  iconUrl: async () => (await import("./superposition-HG6MMR2Y-4XCB2EKG.js")).default
};
var scrollIcon = {
  iconBackground: "#FFEEDA",
  iconUrl: async () => (await import("./scroll-5OBGQVOV-DVW57KGZ.js")).default
};
var unichainIcon = {
  iconBackground: "#F50DB4",
  iconUrl: async () => (await import("./unichain-C5BWO2ZY-CVZXNZGT.js")).default
};
var xdcIcon = {
  iconBackground: "#f9f7ec",
  iconUrl: async () => (await import("./xdc-KJ3TDBYO-AS52CHNT.js")).default
};
var zetachainIcon = {
  iconBackground: "#000000",
  iconUrl: async () => (await import("./zetachain-TLDS5IPW-4XPPTHAA.js")).default
};
var zkSyncIcon = {
  iconBackground: "#f9f7ec",
  iconUrl: async () => (await import("./zksync-DH7HK5U4-RYSUOIUJ.js")).default
};
var zoraIcon = {
  iconBackground: "#000000",
  iconUrl: async () => (await import("./zora-FYL5H3IO-6RB5D5YH.js")).default
};
...
function useFingerprint() {
  const fingerprint = (0, import_react31.useCallback)(() => {
    setRainbowKitVersion({ version: "2.2.10" });
  }, []);
...
var src = async () => (await import("./assets-Q6ZU7ZJ5-EQ4WQQQS.js")).default;
...
var src2 = async () => (await import("./login-UP3DZBGS-G6B2OY3S.js")).default;
...
var signInIcon = async () => (await import("./sign-A7IJEUT5-QGMJDE4C.js")).default;
function SignIn({
  onClose,
  onCloseModal
}) {
...
var chainToExplorerUrl = (chain2) => chain2?.blockExplorers?.default?.url;
...
var src3 = async () => (await import("./connect-UA7M4XW6-PDHLN7KB.js")).default;
...
var src4 = async () => (await import("./create-FASO7PVG-OCNZ4KYQ.js")).default;
...
var src5 = async () => (await import("./refresh-S4T5V5GX-SVMWEKA4.js")).default;
...
var src6 = async () => (await import("./scan-4UYSQ56Q-IUO4THNL.js")).default;
...
function QRCode({
  ...
  const padding2 = "20";
  const size3 = sizeProp - Number.parseInt(padding2, 10) * 2;
...
var getBrowserSrc = async () => {
  ...
  switch (browser) {
    case "Arc":
      return (await import("./Arc-VDBY7LNS-KMBRG6AZ.js")).default;
    case "Brave":
      return (await import("./Brave-BRAKJXDS-AGGW5PKV.js")).default;
    case "Chrome":
      return (await import("./Chrome-65Q5P54Y-MPCC2NYP.js")).default;
    case "Edge":
      return (await import("./Edge-XSPUTORV-PNOHIOL6.js")).default;
    case "Firefox":
      return (await import("./Firefox-AAHGJQIP-32CSEGEH.js")).default;
    case "Opera":
      return (await import("./Opera-KQZLSACL-72727WPX.js")).default;
    case "Safari":
      return (await import("./Safari-ZPL37GXR-F4AZAROJ.js")).default;
    default:
      return (await import("./Browser-76IHF3Y2-GSQO5KDS.js")).default;
  }
};
...
var getPlatformSrc = async () => {
  ...
  switch (platform) {
    case "Windows":
      return (await import("./Windows-PPTHQER6-4IT2VYXK.js")).default;
    case "macOS":
      return (await import("./Macos-MW4AE7LN-P5IRKZ7J.js")).default;
    case "Linux":
      return (await import("./Linux-OO4TNCLJ-2GCVVTLA.js")).default;
    default:
      return (await import("./Linux-OO4TNCLJ-2GCVVTLA.js")).default;
  }
};
    ...
    case "CONNECT": {
      ...
      walletContent = import_react75.default.createElement(Box, null, import_react75.default.createElement(
        ...
        wallets.filter((wallet) => wallet.ready).map((wallet) => {
          return import_react75.default.createElement(Box, { key: wallet.id, width: "60" }, import_react75.default.createElement(WalletButton, { onClose, wallet }));
        })
      ), import_react75.default.createElement(
        Box,
        {
...
function ConnectButton({
  ...
  return ready ? import_react4.default.createElement(ConnectButtonRenderer, null, ({
    ...
    const unsupportedChain = chain2?.unsupported ?? false;
...
var baseAccount2 = ({
  ...
  const { preference, ...optionalConfig } = baseAccount2;
  return {
    id: "baseAccount",
    name: "Base Account",
    shortName: "Base Account",
    rdns: "app.base.account",
    iconUrl: async () => (await import("./baseAccount-44UITRK7-VRULMHNO.js")).default,
    iconAccent: "#0000FF",
...
var metaMaskWallet = ({
  ...
  return {
    ...
    name: "MetaMask",
    rdns: "io.metamask",
    iconUrl: async () => (await import("./metaMaskWallet-SITXT2FV-F4S53EXF.js")).default,
    iconAccent: "#f6851a",
    iconBackground: "#fff",
...
var rainbowWallet = ({
  ...
  return {
    ...
    name: "Rainbow",
    rdns: "me.rainbow",
    iconUrl: async () => (await import("./rainbowWallet-O26YNBMX-RJFVBS6X.js")).default,
...
var safeWallet = () => ({
  ...
  iconAccent: "#12ff80",
  iconBackground: "#fff",
  iconUrl: async () => (await import("./safeWallet-5MNKTR5Z-3LFEMZMX.js")).default,
  installed: (
...
var walletConnectWallet = ({
  ...
  return {
    ...
    name: "WalletConnect",
    installed: void 0,
    iconUrl: async () => (await import("./walletConnectWallet-YHWKVTDY-BM2JFL2Y.js")).default,
