...
async function prepareTransactionRequest(client, args) {
  ...
  if (prepareTransactionRequest3?.fn && prepareTransactionRequest3.runAt?.includes("beforeFillTransaction")) {
  ...
  if (prepareTransactionRequest3?.fn && prepareTransactionRequest3.runAt?.includes("beforeFillParameters")) {
    request = await prepareTransactionRequest3.fn({ ...request, chain }, {
  ...
  if (prepareTransactionRequest3?.fn && prepareTransactionRequest3.runAt?.includes("afterFillParameters"))
    request = await prepareTransactionRequest3.fn({ ...request, chain }, {
...
async function getWebSocketRpcClient(url, options = {}) {
  ...
  return getSocketRpcClient({
    async getSocket({ onClose, onError, onOpen, onResponse }) {
      const WebSocket = await import("./native-JW4MDP55.js").then((module) => module.WebSocket);
...
async function getTransaction(client, { blockHash, blockNumber, blockTag: blockTag_, hash, index: index3, sender, nonce }) {
  const blockTag = blockTag_ || "latest";
  const blockNumberHex = blockNumber !== void 0 ? numberToHex(blockNumber) : void 0;
...
function encode2(...parameters) {
  ...
  return concat2(bytecode, abiConstructor.inputs?.length && args?.length ? encode(abiConstructor.inputs, args) : "0x");
}
function from2(abiConstructor) {
  return from(abiConstructor);
}
...
function encodeData2(...parameters) {
  ...
  const data = args.length > 0 ? encode(item.inputs, args) : void 0;
  return data ? concat2(selector, data) : selector;
}
function from3(abiFunction, options = {}) {
  return from(abiFunction, options);
}
...
async function simulateCalls(client, parameters) {
  ...
  const getBalanceData = account ? encode2(from2("constructor(bytes, bytes)"), {
    bytecode: deploylessCallViaBytecodeBytecode,
    args: [
      getBalanceCode,
      encodeData2(from3("function getBalance(address)"), [account.address])
    ]
  }) : void 0;
      ...
      ...traceAssetChanges ? [
        ...
        // Asset pre balances
        {
          calls: assetAddresses.map((address, i) => ({
            abi: [
              from3("function balanceOf(address) returns (uint256)")
            ],
      ...
      ...traceAssetChanges ? [
        ...
        // Decimals
        {
          calls: assetAddresses.map((address, i) => ({
            to: address,
            abi: [
              from3("function decimals() returns (uint256)")
            ],
        ...
        // Token URI
        {
          calls: assetAddresses.map((address, i) => ({
            to: address,
            abi: [
              from3("function tokenURI(uint256) returns (string)")
            ],
            functionName: "tokenURI",
            args: [0n],
            from: zeroAddress,
        ...
        // Symbols
        {
          calls: assetAddresses.map((address, i) => ({
            to: address,
            abi: [from3("function symbol() returns (string)")],
            functionName: "symbol",
            from: zeroAddress,
            nonce: i
          })),
        ...
        try {
          ...
          if (err instanceof TransactionNotFoundError || err instanceof TransactionReceiptNotFoundError) {
            ...
            try {
              ...
              retrying = false;
              const replacementTransaction = block.transactions.find(({ from: from4, nonce }) => from4 === replacedTransaction.from && nonce === replacedTransaction.nonce);
...
async function sendCalls(client, parameters) {
  const { account: account_ = client.account, chain = client.chain, experimental_fallback, experimental_fallbackDelay = 32, forceAtomic = false, id, version: version3 = "2.0.0" } = parameters;
  const account = account_ ? parseAccount(account_) : null;
            ...
            const { result: result2, error: error2 } = await rpc.http(url, {
              body: {
                ...
                params: [
                  {
                    ...call3,
                    ...typeof from4 !== "undefined" ? { from: from4 } : {}
                  }
                ]
              }
            });
...
function uid2(length = 11) {
  if (!buffer2 || index2 + length > size4 * 2) {
    buffer2 = "";
    index2 = 0;
...
//# sourceMappingURL=chunk-XHO36NMN.js.map
